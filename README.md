...
| The in-app browser | `src-tauri/src/saved.rs`, `src/components/BrowserModal.tsx` | The modal is chrome; the page is a real webview the backend owns, overlaid on the rectangle the modal's body reports and moved with it by a `ResizeObserver`. That rectangle comes from `getBoundingClientRect`, which is relative to the *page*, while a child webview is placed relative to the *window* — and on a Mac those differ by the title bar, since the window has a full-size content view and the page starts below it. So the main webview's own position is read back and added: a correction where one is needed, zero where it isn't, rather than a guess at a title bar's height. Desktop uses a Tauri child webview (`Window::add_child`, the `unstable` multiwebview API); iPadOS a native `WKWebView` from `tauri-plugin-saved-browser`, since child webviews are desktop-only. An init script folds `window.open` and `target=_blank` back into the same webview, which a Tauri webview otherwise swallows. The page is remote, so no capability covers it and it can't call a command |
...
| The browser covers the modal's own header | The page and the window disagree about where the top is, and the offset between them came out wrong | The log names the rectangle the browser was placed at and the offset applied to get there |
