...
1. **Choose an input** — where this session's reading comes from, at the top of
   ...
   mailbox; **Saved List** opens a site in a browser and takes the list you
   point it at. A session collects from one of them, so switching inputs clears
...
other site with a *save*, a *read later* or a *bookmarks* page.
...
### The session, and paywalled posts
...
none of this app's do, so nothing loaded in there can call a command.

### Reading the articles

A collected article is fetched and scraped by the same code that reads the links
out of an AI-agent newsletter: the body copy is located, taken as Markdown with
...
*Remember removed content* likewise belongs to a mail filter, and a saved article
was found by no filter — striking things out of one works, but nothing is
remembered between runs.

### The browser itself

Two implementations of the same thing, because a webview is platform machinery:

| | |
| --- | --- |
| **macOS** | A Tauri child webview added to the main window (`Window::add_child`, the `unstable` multiwebview API), positioned over the modal's body and moved with it |
| **iPadOS** | A native `WKWebView` overlaid at the same rectangle by `tauri-plugin-saved-browser`, since child webviews are desktop-only |

Both run the *same* harvest script — it's defined once in `src-tauri/src/saved.rs`
and handed to whichever side is doing the work — and both hand back the page's
address, the articles on it, and the session cookie. The cookie comes from the
webview's own store rather than the page, since a session cookie is `HttpOnly`
and the script couldn't see it either.
...
The **Saved List** input needs no Google setup at all — you sign in to the site
itself, in the browser the app opens. It does need one iPad-specific piece:
Tauri's child webviews are desktop-only, so on iPadOS the browser is a native
`WKWebView` overlay provided by `src-tauri/tauri-plugin-saved-browser`, a local
Tauri plugin with a Swift half. It's wired up like any other plugin (already in
`Cargo.toml` and `lib.rs`) and `tauri ios init` picks up its `ios/Package.swift`
without further configuration.

> As with the deep-link sign-in above, the iPad half of the browser is written
> and type-checks, but the Swift package can only be compiled and run on a Mac
> with Xcode, which isn't available in this build environment. The desktop
> child-webview path is the one exercised here.
...
| The in-app browser | `src-tauri/src/saved.rs`, `src/components/BrowserModal.tsx` | The modal is chrome; the page is a real webview the backend owns, overlaid on the rectangle the modal's body reports and moved with it by a `ResizeObserver`. Two coordinate spaces meet here: the rectangle is measured in the *page*, and the browser is placed in the *window*, which on a Mac starts a title bar higher. Rather than assume that furniture's height — or assume there is any — the page sends how tall it is and the backend subtracts it from the window's client area; the gap is the correction, and it comes out zero where the two already agree. The log names both heights, so a browser that lands wrong says why. Its own header states its height rather than deriving one: as a flex item that was also a flex container, the bar's height came from resolving `flex-basis: auto` against its contents — so the outer element carries the height and an inner row arranges the controls in it. Desktop uses a Tauri child webview (`Window::add_child`, the `unstable` multiwebview API); iPadOS a native `WKWebView` from `tauri-plugin-saved-browser`, since child webviews are desktop-only. An init script folds `window.open` and `target=_blank` back into the same webview, which a Tauri webview otherwise swallows. The page is remote, so no capability covers it and it can't call a command |
...
| The browser covers the modal's own header | The page and the window disagree about where the top is by more than the app worked out | The log's first `saved` line gives both heights and the offset it applied — the difference between them is the number that's wrong |

## Privacy

- Read-only Gmail scope; nothing is written to your mailbox.
- OAuth tokens are stored in the app's local data directory only.
- The **saved list** browser is a real browser: you sign in to the site itself,
  and the app never sees your password. What it keeps afterwards is that site's
  session cookie, in the same local directory. It grants whatever your account
  can do, so it's handled as a password: written to the log by name only, never
  its value, and sent to that site's domain and its subdomains alone — never to
  an article hosted somewhere else. **Forget the … session** discards it.
- The page in the browser is a plain remote page with no access to the app.
  Tauri capabilities apply to local app URLs unless a capability names remote
  URLs, and none of this app's do, so nothing loaded in there can call a command.
- Collecting a list talks to that site and to the sites its articles are on, and
  ...
  nothing at all until you open the browser.
